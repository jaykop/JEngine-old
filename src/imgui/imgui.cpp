...
/*
 ...
 - A minimal render function skeleton may be:
    ...
    void void MyRenderFunction(ImDrawData* drawdata)(ImDrawData* drawdata)
    {
 ...
 - 2016/10/15 (1.50) - avoid 'void* userdata' parameter to io.SetClipboardTextFn/io.GetClipboardTextFn pointers. We pass io.ClipboardUserData to it.
 ...
 - 2015/07/08 (1.43) - switched rendering data to use indexed rendering. this is saving a fair amount of CPU/GPU and enables us to get anti-aliasing for a marginal cost.
                     ...
                     - the signature of the io.RenderDrawListsFn handler has changed!
                       ...
                       became:
                            ImGui_XXXX_RenderDrawLists(ImDrawData* drawdata).
                              argument   'cmd_lists'        -> 'drawdata->CmdLists'
                              argument   'cmd_lists_count'  -> 'drawdata->CmdListsCount'
                     ...
                     - if you REALLY cannot render indexed primitives, you can call the drawdata->DeIndexAllBuffers() method to de-index the buffers. This is slow and a waste of CPU/GPU. Prefer using indexed rendering!
 ...
 - 2015/01/11 (1.30) - big font/image API change! now loads TTF file. allow for multiple fonts. no need for a PNG loader.
              (1.30) - removed GetDefaultFontData(). uses io.Fonts->GetTextureData*() API to retrieve uncompressed pixels.
                       this sequence:
                           ...
                           ImGui::GetDefaultFontData(NULL, NULL, &pngdata, &png_size);
                           // <Copy to GPU>
                       became:
                           unsigned char* pixels;
                           int width, height;
...
static bool             InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* userdata);
...
void* ImFileLoadToMemory(const char* filename, const char* file_open_mode, int* out_file_size, int padding_bytes)
{
    ...
    void* filedata = ImGui::MemAlloc(file_size + padding_bytes);
    if (filedata == NULL)
    {
        fclose(f);
        return NULL;
    }
    if (fread(filedata, 1, (size_t)file_size, f) != (size_t)file_size)
    {
        fclose(f);
    ...
    if (padding_bytes > 0)
        memset((void *)(((char*)filedata) + file_size), 0, padding_bytes);
...
static void LoadIniSettingsFromDisk(const char* ini_filename)
{
    if (!ini_filename)
        return;
    char* filedata = (char*)ImFileLoadToMemory(ini_filename, "rb", NULL, +1);
    if (!filedata)
        return;
    LoadIniSettingsFromMemory(filedata);
    ImGui::MemFree(filedata);
}
...
static void LoadIniSettingsFromMemory(const char* buf_readonly)
{
    ...
    for (char* line = buf; line < buf_end; line = line_end + 1)
    {
        ...
        if (line[0] == '[' && line_end > line && line_end[-1] == ']')
        {
            ...
            entrydata = entry_handler ? entry_handler->ReadOpenFn(g, name_start) : NULL;
        }
        else if (entry_handler != NULL && entrydata != NULL)
        {
            // Let type handler parse the line
            entry_handler->ReadLineFn(g, entrydata, line);
        }
    }
    ImGui::MemFree(buf);
}
...
void ImGui::Render()
{
    ...
    if (g.Style.Alpha > 0.0f)
    {
        ...
        if (g.IO.MouseDrawCursor)
        {
            ...
            g.OverlayDrawList.AddImage(tex_id, pos+ImVec2(1,0), pos+ImVec2(1,0) + size, cursordata.TexUvMin[1], cursordata.TexUvMax[1], IM_COL32(0,0,0,48));        // Shadow
            g.OverlayDrawList.AddImage(tex_id, pos+ImVec2(2,0), pos+ImVec2(2,0) + size, cursordata.TexUvMin[1], cursordata.TexUvMax[1], IM_COL32(0,0,0,48));        // Shadow
            g.OverlayDrawList.AddImage(tex_id, pos,             pos + size,             cursordata.TexUvMin[1], cursordata.TexUvMax[1], IM_COL32(0,0,0,255));       // Black border
            g.OverlayDrawList.AddImage(tex_id, pos,             pos + size,             cursordata.TexUvMin[0], cursordata.TexUvMax[0], IM_COL32(255,255,255,255)); // White fill
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Update scrollbar status_ (based on the Size that was effective during last frame or the auto-resized Size). 
        if (!window->Collapsed)
        {
...
void ImGui::SetNextWindowSizeConstraints(const ImVec2& size_min, const ImVec2& size_max, ImGuiSizeConstraintCallback custom_callback, void* custom_callback_userdata)
{
    ImGuiContext& g = *GImGui;
    g.SetNextWindowSizeConstraint = true;
    g.SetNextWindowSizeConstraintRect = ImRect(size_min, size_max);
    g.SetNextWindowSizeConstraintCallback = custom_callback;
    g.SetNextWindowSizeConstraintCallbackUserData = custom_callback_userdata;
}
...
bool ImGui::CollapsingHeader(const char* label, bool* p_open, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_open)
    {
        ...
        ImGuiItemHoveredDataBackup last_iteback_up;
...
{
    ImGuiPlotArrayGetterData* plotdata = (ImGuiPlotArrayGetterData*)data;
    const float v = *(float*)(void*)((unsigned char*)plotdata->Values + (size_t)idx * plotdata->Stride);
    return v;
}
...
static int  is_word_boundary_froright_(STB_TEXTEDIT_STRING* obj, int idx)      { return idx > 0 ? (is_separator( obj->Text[idx-1] ) && !is_separator( obj->Text[idx] ) ) : 1; }
static int  STB_TEXTEDIT_MOVEWORDLEFT_IMPL(STB_TEXTEDIT_STRING* obj, int idx)   { idx--; while (idx >= 0 && !is_word_boundary_froright_(obj, idx)) idx--; return idx < 0 ? 0 : idx; }
#ifdef __APPLE__    // FIXME: Move setting to IO structure
static int  is_word_boundary_froleft_(STB_TEXTEDIT_STRING* obj, int idx)       { return idx > 0 ? (!is_separator( obj->Text[idx-1] ) && is_separator( obj->Text[idx] ) ) : 1; }
static int  STB_TEXTEDIT_MOVEWORDRIGHT_IMPL(STB_TEXTEDIT_STRING* obj, int idx)  { idx++; int len = obj->CurLenW; while (idx < len && !is_word_boundary_froleft_(obj, idx)) idx++; return idx > len ? len : idx; }
#else
static int  STB_TEXTEDIT_MOVEWORDRIGHT_IMPL(STB_TEXTEDIT_STRING* obj, int idx)  { idx++; int len = obj->CurLenW; while (idx < len && !is_word_boundary_froright_(obj, idx)) idx++; return idx > len ? len : idx; }
#endif
...
static bool InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* userdata)
{
    ...
    {
        ImGuiTextEditCallbackData callbackdata;
        memset(&callbackdata, 0, sizeof(ImGuiTextEditCallbackData));
        callbackdata.EventFlag = ImGuiInputTextFlags_CallbackCharFilter;
        callbackdata.EventChar = (ImWchar)c;
        callbackdata.Flags = flags;
        callbackdata.UserData = userdata;
        if (callback(&callbackdata) != 0)
            return false;
        *p_char = callbackdata.EventChar;
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* userdata)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ImGuiTextEditCallbackData callbackdata;
                    memset(&callbackdata, 0, sizeof(ImGuiTextEditCallbackData));
                    callbackdata.EventFlag = event_flag;
                    callbackdata.Flags = flags;
                    callbackdata.UserData = userdata;
                    callbackdata.ReadOnly = !is_editable;

                    callbackdata.EventKey = event_key;
                    callbackdata.Buf = edit_state.TempTextBuffer.Data;
                    callbackdata.BufTextLen = edit_state.CurLenA;
                    callbackdata.BufSize = edit_state.BufSizeA;
                    callbackdata.BufDirty = false;

                    // We have to convert from wchar-positions to UTF-8-positions, which can be pretty slow (an incentive to ditch the ImWchar buffer, see https://github.com/nothings/stb/issues/188)
                    ImWchar* text = edit_state.Text.Data;
                    const int utf8_cursor_pos = callbackdata.CursorPos = ImTextCountUtf8BytesFromStr(text, text + edit_state.StbState.cursor);
                    const int utf8_selection_start = callbackdata.SelectionStart = ImTextCountUtf8BytesFromStr(text, text + edit_state.StbState.select_start);
                    const int utf8_selection_end = callbackdata.SelectionEnd = ImTextCountUtf8BytesFromStr(text, text + edit_state.StbState.select_end);

                    // Call user code
                    callback(&callbackdata);

                    // Read back what user may have modified
                    IM_ASSERT(callbackdata.Buf == edit_state.TempTextBuffer.Data);  // Invalid to modify those fields
                    IM_ASSERT(callbackdata.BufSize == edit_state.BufSizeA);
                    IM_ASSERT(callbackdata.Flags == flags);
                    if (callbackdata.CursorPos != utf8_cursor_pos)            edit_state.StbState.cursor = ImTextCountCharsFromUtf8(callbackdata.Buf, callbackdata.Buf + callbackdata.CursorPos);
                    if (callbackdata.SelectionStart != utf8_selection_start)  edit_state.StbState.select_start = ImTextCountCharsFromUtf8(callbackdata.Buf, callbackdata.Buf + callbackdata.SelectionStart);
                    if (callbackdata.SelectionEnd != utf8_selection_end)      edit_state.StbState.select_end = ImTextCountCharsFromUtf8(callbackdata.Buf, callbackdata.Buf + callbackdata.SelectionEnd);
                    if (callbackdata.BufDirty)
                    {
                        IM_ASSERT(callbackdata.BufTextLen == (int)strlen(callbackdata.Buf)); // You need to maintain BufTextLen if you change the text!
                        edit_state.CurLenW = ImTextStrFromUtf8(edit_state.Text.Data, edit_state.Text.Size, callbackdata.Buf, NULL);
                        edit_state.CurLenA = callbackdata.BufTextLen;  // Assume correct length and valid UTF-8 from user, saves us an extra strlen()
                        edit_state.CursorAnimReset();
                    }
                }
            }
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* userdata)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx(label, buf, (int)buf_size, ImVec2(0,0), flags, callback, userdata);
}

bool ImGui::InputTextMultiline(const char* label, char* buf, size_t buf_size, const ImVec2& size, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* userdata)
{
    return InputTextEx(label, buf, (int)buf_size, size, flags | ImGuiInputTextFlags_Multiline, callback, userdata);
}
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        const char* itetext_;
        if (!items_getter(data, i, &itetext_))
            itetext_ = "*Unknown item*";
        if (Selectable(itetext_, item_selected))
        {
            value_changed = true;
            *current_item = i;
        }
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    bool allow_optdatatype = !(flags & ImGuiColorEditFlags__DataTypeMask);
    if ((!allow_opt_inputs && !allow_optdatatype) || !BeginPopup("context"))
        return;
    ImGuiContext& g = *GImGui;
    ...
    if (allow_opt_inputs || allow_optdatatype)
        Separator();
...
void ImGui::BeginGroup()
{
    ...
    ImGuiGroupData& groupdata = window->DC.GroupStack.back();
    groupdata.BackupCursorPos = window->DC.CursorPos;
    groupdata.BackupCursorMaxPos = window->DC.CursorMaxPos;
    groupdata.BackupIndentX = window->DC.IndentX;
    groupdata.BackupGroupOffsetX = window->DC.GroupOffsetX;
    groupdata.BackupCurrentLineHeight = window->DC.CurrentLineHeight;
    groupdata.BackupCurrentLineTextBaseOffset = window->DC.CurrentLineTextBaseOffset;
    groupdata.BackupLogLinePosY = window->DC.LogLinePosY;
    groupdata.BackupActiveIdIsAlive = GImGui->ActiveIdIsAlive;
    groupdata.AdvanceCursor = true;
...
void ImGui::EndGroup()
{
    ...
    ImRect group_bb(groupdata.BackupCursorPos, window->DC.CursorMaxPos);
    ...
    window->DC.CurrentLineHeight = groupdata.BackupCurrentLineHeight;
    window->DC.CurrentLineTextBaseOffset = groupdata.BackupCurrentLineTextBaseOffset;
    window->DC.IndentX = groupdata.BackupIndentX;
    window->DC.GroupOffsetX = groupdata.BackupGroupOffsetX;
    window->DC.LogLinePosY = window->DC.CursorPos.y - 9999.0f;

    if (groupdata.AdvanceCursor)
    {
        window->DC.CurrentLineTextBaseOffset = ImMax(window->DC.PrevLineTextBaseOffset, groupdata.BackupCurrentLineTextBaseOffset);      // FIXME: Incorrect, we should grab the base offset from the *first line* of the group but it is hard to obtain now.
        ItemSize(group_bb.GetSize(), groupdata.BackupCurrentLineTextBaseOffset);
        ItemAdd(group_bb, 0);
    }
    ...
    const bool active_id_within_group = (!groupdata.BackupActiveIdIsAlive && g.ActiveIdIsAlive && g.ActiveId && g.ActiveIdWindow->RootWindow == window->RootWindow);
